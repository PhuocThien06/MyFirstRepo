...
bool addHead(List*& L, int data) {
    ...
    L->p_head = newNode;
    if (L->p_tail == nullptr) L->p_tail = newNode; 
    return true;
}
bool addTail(List*& L, int data) {
    NODE* newNode = createNode(data);
...
void removeBefore(List*& L, int val) {
    if (L == nullptr || L->p_head == nullptr || L->p_head->p_next == nullptr) return;
    if (L->p_head->p_next->key == val) {
        removeHead(L);
        return;
    }
    NODE* current = L->p_head;
    while (current->p_next != nullptr && current->p_next->p_next != nullptr) {
        if (current->p_next->p_next->key == val) {
...
void removeAfter(List*& L, int val) {
    ...
    NODE* current = L->p_head;
    while (current != nullptr) {
        if (current->key == val && current->p_next != nullptr) {
            NODE* temp = current->p_next;
            current->p_next = temp->p_next;
            if (temp == L->p_tail) {
                L->p_tail = current;
...
int countElements(List* L) {
    if (L == nullptr || L->p_head == nullptr) return 0;
...
List* reverseList(List* L) {
    if (L == nullptr || L->p_head == nullptr) return createList(nullptr);

    NODE* current = L->p_head;
    List* newList = createList(nullptr);

    while (current != nullptr) {
        NODE* newNode = new NODE{ current->key, nullptr };
        newNode->p_next = newList->p_head;
        newList->p_head = newNode;
        if (newList->p_tail == nullptr) {
