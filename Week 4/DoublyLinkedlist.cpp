...
// 2
d_List* createList(d_NODE* p_node) {
    d_List* newList = new d_List(p_node, p_node);
...
void removeBefore(d_List*& L, int val) {
    d_NODE* p = L->pHead;
    while (p != NULL && p->key != val) p = p->pNext;
    if (p != NULL && p->pPrev != NULL) {
        d_NODE* q = p->pPrev;
        if (q == L->pHead) L->pHead = p;
        else q->pPrev->pNext = p;
...
bool addPos(d_List*& L, int data, int pos) {
    ...
    if (newNode->pNext == nullptr) L->pTail = newNode;
    return true;
}
// 11
void removePos(d_List*& L, int data, int pos) {
    if (L->pHead == NULL || pos < 0) return;
    d_NODE* p = L->pHead;
    for (int i = 0; i < pos && p != NULL; i++) p = p->pNext;
    if (p == NULL) return;
    if (p == L->pHead) removeHead(L);
    else if (p == L->pTail) removeTail(L);
    else {
...
// 17
void removeDuplicate(d_List*& L) {
    for (d_NODE* p = L->pHead; p != NULL; p = p->pNext) {
        d_NODE* q = p->pNext;
        while (q != NULL) {
            if (q->key == p->key) {
                d_NODE* toDelete = q;
                ...
                if (toDelete == L->pHead) L->pHead = toDelete->pNext;
...
bool removeElement(d_List*& L, int key) {
    ...
    while (p != NULL) {
        if (p->key == key) {
            d_NODE* toDelete = p;
            p = p->pNext;
            if (toDelete == L->pHead) removeHead(L);
            else if (toDelete == L->pTail) removeTail(L);
            else {
